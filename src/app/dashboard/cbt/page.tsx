...
import { CBT_WORKSHEETS } from "@/lib/cbt-worksheets";
...
export default function CbtToolboxPage() {
  const [activeType, setActiveType] = useState(CBT_WORKSHEETS[0].type);
  const def = CBT_WORKSHEETS.find((w) => w.type === activeType)!;
  const [formData, setFormData] = useState<Record<string, string>>({});
  const [history, setHistory] = useState<Worksheet[]>([]);
  const [saving, setSaving] = useState(false);

  async function loadHistory(type: string) {
    const res = await fetch(`/api/cbt?type=${type}`);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-display text-3xl font-medium">CBT toolbox</h1>
        <p className="mt-1 text-navy-500 dark:text-white/60">
          Structured, evidence-informed worksheets you can revisit any time.
        </p>
      </div>

      <div className="flex flex-wrap gap-2">
        {CBT_WORKSHEETS.map((w) => (
          <button
            key={w.type}
